 ...
 */
/**
 * Determine if schema contains image data - Optimized to avoid full stringification
 */
const hasImageData = (schema: any): boolean => {
  if (!schema) return false;

  // Basic object property checks are much faster than JSON.stringify for large lists
  if (schema.image || schema.thumbnailUrl || schema.thumbnail) return true;

  // Recursive check for nested items (like in ItemList)
  if (schema.itemListElement && Array.isArray(schema.itemListElement)) {
    // Just check the first item as a heuristic for the whole list
    const firstItem = schema.itemListElement[0]?.item || schema.itemListElement[0];
    return firstItem && (firstItem.image || firstItem.thumbnail);
  }
...
export function MultipleSchemas({
  ...
  schemas.forEach((schema) => {
    ...
    // Calculate priority and defer status once
    const priority = getSchemaPriority(schema);
    const isImageSchema = hasImageData(schema);
    const shouldDefer = deferImageSchemas && isImageSchema;
