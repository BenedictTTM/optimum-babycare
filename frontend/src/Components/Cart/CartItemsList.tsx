...
interface CartItemsListProps {
  ...
  confirmRemoveId?: number | string | null;
  onCancelRemove?: () => void;
}
    ...
    <div className="bg-white rounded-md border border-gray-100 overflow-hidden">
      {items.map((item, index) => (
        <CartItem
          ...
          isConfirmingRemove={confirmRemoveId === item.id}
  ...
  return (
    <div
      className={`p-2 sm:p-2.5 md:p-3 ${!isLastItem ? 'border-b border-gray-100' : ''
        } ${isUpdating ? 'opacity-50 pointer-events-none' : ''} ${isSelected ? 'bg-red-50 border-l-2 border-l-amber-400' : ''
        }`}
    >
        ...
        {showSelection && onSelect && (
          <div className="flex items-start pt-1 flex-shrink-0">
            <input
              ...
              checked={isSelected}
              onChange={onSelect}
              className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400 focus:ring-amber-400 cursor-pointer"
      ...
      {isConfirmingRemove && (
        <div className="mt-2 flex items-center gap-2 bg-red-50 rounded-md px-3 py-2">
          <span className="text-xs text-red-700 font-medium">Remove this item?</span>
          <button
          ...
          <button
            ...
            className="px-2.5 py-1 text-xs font-semibold text-gray-600 bg-gray-200 hover:bg-gray-300 rounded transition-colors"
          >
            Cancel
          </button>
        </div>
  ...
  return (
    <div className="flex justify-between gap-2 mb-2">
      ...
      <button
        onClick={onRemove}
        disabled={isUpdating}
        className="text-gray-400 hover:text-amber-400 transition-colors disabled:opacity-50 flex-shrink-0 sm:hidden"
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3 min-w-0">
      ...
      <button
        onClick={onRemove}
        disabled={isUpdating}
        className="text-gray-400 hover:text-amber-400 transition-colors disabled:opacity-50 hidden sm:block flex-shrink-0"
        aria-label="Remove item"
      >
        {isUpdating ? (
