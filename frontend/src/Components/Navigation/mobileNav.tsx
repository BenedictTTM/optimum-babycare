...
const Sidebar = ({ onClose }: MobileNavProps) => {
  ...
  useEffect(() => {
    // Ensure user data is loaded for nav display
    fetchUser().catch(() => { });
  }, [fetchUser]);
  ...
  const navItems = [
    { id: 'home', label: 'Dashboard', icon: Home, path: '/accounts', hasSubmenu: false },
    {
      ...
      submenu: [
        ...
        { id: 'categories', label: 'Categories', icon: Folder, path: '/accounts/addCategories' },
      ]
    },
  ...
  return (
    <>
      ...
      <div
        onClick={() => onClose?.()}
        aria-hidden="true"
        className="fixed inset-0  backdrop-blur-xs transition-opacity duration-200 z-40"
      />
      ...
      <div className="fixed left-0 top-0 h-screen w-full max-w-sm bg-white text-gray-800 shadow-2xl flex flex-col overflow-hidden z-50">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              {profile?.profilePic ? (
                ...
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-red-400 to-amber-700 flex items-center justify-center text-white font-semibold text-sm shadow-md">
                  {initials}
                </div>
              )}
              <div>
                ...
                <li key={item.id}>
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3.5 rounded-lg transition-all duration-200 ${isActive && !item.hasSubmenu
                      ? 'bg-red-50 text-amber-700 font-semibold shadow-sm'
                  ...
                  >
                    ...
                    {item.hasSubmenu && (
                      <ChevronDown
                        size={16}
                        strokeWidth={2}
                        className={`text-gray-400 transition-transform duration-200 ${openSubmenu ? 'rotate-180' : ''
                          }`}
                      />
                    )}
                  </button>
        ...
        <div className="px-5 py-4 border-t border-gray-100 bg-gradient-to-br from-gray-50 to-white">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gray-700 hover:bg-gray-100 hover:text-amber-700 transition-colors"
          >
            <LogOut size={18} />
