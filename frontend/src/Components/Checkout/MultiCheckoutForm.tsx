...
export default function MultiCheckoutForm({ items, currency, subtotal, hall, whatsapp, callNumber, message, errors, isSubmitting, onChange, onConfirm, onBack }: Props) {
  return (
    <div className="bg-white rounded-xl shadow-sm p-4 sm:p-5 md:p-6 space-y-4 md:space-y-5 md:py-17">
      <div>
        <h2 className="font-semibold text-gray-800 mb-2 text-sm sm:text-base">Order Summary</h2>
        <div className="space-y-2">
          {items.map((it) => (
            <div key={it.product.id} className="flex justify-between items-start border border-gray-100 rounded-lg px-3 py-2 sm:px-4 sm:py-3 bg-gray-50">
              <div className="flex-1">
                <p className="text-gray-700 font-medium text-[13px] sm:text-sm md:text-base">{it.product.title}</p>
                <p className="text-[11px] sm:text-xs md:text-sm text-gray-500 mt-1">Quantity: {it.quantity}</p>
              </div>
              <span className="font-semibold text-gray-800 text-sm sm:text-base">{currency} {(it.product.discountedPrice ?? it.product.originalPrice ?? 0).toFixed(2)}</span>
            </div>
          ))}
        </div>
      </div>
