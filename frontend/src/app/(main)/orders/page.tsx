  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      if ((result as any).status === 401) {
        ...
        const target = encodeURIComponent(pathname || "/orders");
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 mb-4 md:mb-6">
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 py-2 text-xs sm:text-sm text-amber-400 hover:text-gray-800 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
          >
            Continue Shopping
          </button>
        </div>
                ...
                <div className="px-4 sm:px-5 py-3 sm:py-4">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <a
                      ...
                      className="flex-1 inline-flex items-center justify-center gap-2 text-amber-400 text-white font-semibold py-2.5 sm:py-3 px-4 rounded-lg transition-colors"
                    >
                      <svg className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <span className="hidden sm:inline text-amber-400 ">Lets Talk</span>
                    </a>

                    {order.paymentStatus === 'UNPAID' && (
                      <button
                        onClick={() => router.push(`/orders/${order.id}/pay`)}
                        className="flex-1 bg-amber-400 hover:bg-amber-800 text-white font-semibold py-2.5 sm:py-3 px-4 rounded-lg transition-colors flex items-center justify-center gap-2 shadow-xs"
                      >
